...
const ConferenceBookings: React.FC<BookingListProps> = ({ bookings }) => {
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'ConferenceBookings');
    const buf = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    saveAs(new Blob([buf], { type: 'application/octet-stream' }), `conference_bookings_${exportFrom || 'all'}_${exportTo || 'all'}.xlsx`);
  };
  ...
  console.log('ConferenceBookings received:', bookings);
  ...
  useEffect(() => {
    const conferenceIds = Array.from(new Set(bookings.map(b => b.conference_id)));
  ...
  const conference = conferenceDetails[booking.conference_id];
            ...
            <>
              {activeTab === 'confirmed' ? (
                <div className="space-y-4">
                  {filteredBookings.filter(b => b.status === 'confirmed').length > 0 ? filteredBookings.filter(b => b.status === 'confirmed').map(renderBooking) : <div className="text-gray-500">No confirmed conference bookings.</div>}
                </div>
              ) : (
                <div className="space-y-4">
                  {filteredBookings.filter(b => b.status !== 'confirmed').length > 0 ? filteredBookings.filter(b => b.status !== 'confirmed').map(renderBooking) : <div className="text-gray-500">No pending conference bookings.</div>}
                </div>
              )}
            </>
          ) : (
